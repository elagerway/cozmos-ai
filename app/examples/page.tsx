...
export default function ExamplesPage() {
  ...
  // Auto-refresh: poll Supabase every 5 seconds for new/updated spheres
  useEffect(() => {
    const refreshFromSupabase = () => {
      fetchGenerations(true).then((rows) => {
        const generated: Example[] = rows.map((r: GenerationRow) => ({
          id: r.id, prompt: r.prompt, status: (r.status || "done") as any,
          step: (r.step || "done") as any, step_label: r.step_label,
          sphere_spec: null, bg_prompt: null, image_url: r.image_url,
          error: null, cost_usd: r.cost_usd ? Number(r.cost_usd) : null,
          duration_s: r.duration_s, created_at: r.created_at,
          featured: false, environment: "pipeline",
    ...
    // Check if there are any running items — if so, poll frequently
    const hasRunning = allExamples.some((e) => e.status === "running")
    const interval = setInterval(refreshFromSupabase, hasRunning ? 5000 : 30000)
